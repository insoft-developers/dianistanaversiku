...
$(document).ready(function() {
    ...
    btnAdd = '&emsp;&emsp; <button type="button" class="btn btn-outline-primary btn-sm" title="Tambah Data Kategori Penyelia" id="btnAdd"> <i class="fas fa-plus"></i> Tambah Kategori</button>';
    ...
    listDataTables("listTable","penyelia-kategori-list",btnRefresh,btnAdd);
    ...
    listDataTables("listTableTrash","penyelia-kategori-list-trash",btnRefreshTrash);
})
...
$(document).on('click', '#btnAdd', function(e) {
    ...
    $("#formData")[0].reset();
    $(".modal-title").text("Tambah Kategori Penyelia");
    save_method = "add";
...
function editData(id) {
    ...
    $("#formData")[0].reset();
    $("#modalForm").modal("show");
    $(".modal-title").text("Edit Kategori Penyelia");
    save_method = "update";
    $("input[name='_method']").val("PUT");

    $.get(url_back+"/penyelia-kategori/"+idData+"/edit", function(resp) {
        if (resp.status == true) {
            $("#name_kategori").val(resp.data.name_kategori);
        } else {
            setTimeout(function() {
...
$("#modalBtnSave").click(function() {
    ...
    if (save_method == "add") {
        var url = url_back+'/penyelia-kategori';
    } else {
        var url = url_back+'/penyelia-kategori/'+idData;
    }
    ...
    $.get(url_back+"/penyelia-kategori/"+idData+"/edit", function(resp) {
        if (resp.status == true) {
            ...
            msg += '<ul class="text-red">'
                    +'<li class="float-start"><small>Nama Kategori : <i>'+resp.data.name_kategori+'</i></small></li><br>'
                +'</ul>';
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: url_back+"/penyelia-kategori/"+idData,
                        type: 'DELETE',
                        dataType: 'JSON',
                        headers: {'X-CSRF-TOKEN': tokenCsrf},
                        success: function(resp) {
...
function restoreData(id) {
    idData = id;
    $.get(url_back+"/penyelia-kategori/"+idData+"/trash", function(resp) {
        if (resp.status == true) {
            var msg = "";
            msg += '<ul class="text-red">'
                    +'<li class="float-start"><small>Nama Kategori : <i>'+resp.data.name_kategori+'</i></small></li><br>'
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: url_back+"/penyelia-kategori/"+idData+"/restore",
                        type: 'POST',
                        dataType: 'JSON',
                        headers: {'X-CSRF-TOKEN': tokenCsrf},
                        success: function(resp) {
