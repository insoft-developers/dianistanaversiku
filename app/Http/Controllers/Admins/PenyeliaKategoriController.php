...
class PenyeliaKategoriController extends Controller
{
    ...
    {
        return view("admins.penyelia_kategori.index");
    }
    
    private static function set_ajax_list($request,bool $trash=false)
    {
        self::dataTablesQuery(PenyeliaKategori::query());
        self::dataTablesRequest($request);
        self::dataTablesOrderBy([null,'name_kategori']);
        self::dataTablesSearch(['name_kategori']);
        if ($trash) {
    ...
    {
        $rules = [
            'name_kategori'  =>  'required|unique:penyelia_kategori',
        ];
        ...
        if ($validator !== null) {
            ...
            $insert = PenyeliaKategori::create($data);
            if ($insert) {
    ...
    {
        $select = ["name_kategori"];
        $getData = PenyeliaKategori::getFirstQuery(select: $select, where: ["id" => $id]);
        if ($getData) {
    ...
    {
        $getData = PenyeliaKategori::getFirstOnlyTrashed(["id" => $id]);
        if ($getData) {
            Resp::success('data user per id', $getData);
        } else {
            Resp::error(alertDanger('Data Tidak Ada'));
    ...
    {
        $getData = PenyeliaKategori::getFirst(["id" => $id]);
        if ($getData) {
            $rules = [
            ...
            if ($validator !== null) {
                ...
                $update = PenyeliaKategori::updateWhere(["id" => $getData->id],$data);
                if ($update) {
    ...
    public function destroy(string $id)
    {
        $getData = PenyeliaKategori::getFirst(["id" => $id]);
        if ($getData) {
            $delete = PenyeliaKategori::deleteWhere(["id" => $id]);
            if ($delete) {
    ...
    public function restore(string $id)
    {
        $getData = PenyeliaKategori::getFirstOnlyTrashed(["id" => $id]);
        if ($getData) {
            $restore = PenyeliaKategori::restoreOnlyTrashed(["id" => $id]);
            if ($restore) {
